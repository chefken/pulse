...
class ScoreProvider extends ChangeNotifier {
  ...
  List<DayRecord> get last30Days {
    ...
    return allRecords.where((r) {
      final date = DateTime.parse(r.dateKey);
      return date.isAfter(cutoff);
    }).toList();
  }

  Future<void> init() async {
    if (!Hive.isAdapterRegistered(3)) Hive.registerAdapter(DayRecordAdapter());
  ...
  Future<void> updateTodayScore({
    ...
    DayRecord record = _records[key] ?? DayRecord.create(key);
    record
